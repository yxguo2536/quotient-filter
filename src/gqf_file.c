                  ...
                  const char *filename)
{
    ...
    qf->runtimedata->container_resize = cqf_resize_file;
...
uint64_t cqf_usefile(CQF *qf, const char *filename, int flag)
{
    ...
    if (qf->metadata->magic_endian_number != MAGIC_NUMBER) {
        fprintf(stderr,
                "Can't read the CQF. It was written on a different endian "
                "machine.");
        exit(EXIT_FAILURE);
    }
    qf->blocks = (qfblock *) (qf->metadata + 1);
...
int64_t cqf_resize_file(CQF *qf, uint64_t nslots)
{
    ...
    CQF new_qf;
    if (!cqf_initfile(&new_qf, nslots, qf->metadata->key_bits,
    ...
    QFi qfi;
    cqf_iterator_from_position(qf, &qfi, 0);
    int64_t ret_numkeys = 0;
    do {
        uint64_t key, value, count;
        cqfi_get_hash(&qfi, &key, &value, &count);
        cqfi_next(&qfi);
        int ret =
...
#define ROUND_TO_PAGE_GROUP(p)   \
    ((char *) (((intptr_t)(p)) - \
               (((intptr_t)(p)) % (page_size * MADVISE_GRANULARITY))))

static void make_madvise_calls(const CQF *qf, uint64_t oldrun, uint64_t newrun)
{
...
/* This wraps cqfi_next, using madvise(DONTNEED) to reduce our RSS.
   Only valid on mmapped QFs, i.e. cqfs from cqf_initfile and
   cqf_usefile. */
int cqfi_next_madvise(QFi *qfi)
{
    uint64_t oldrun = qfi->run;
    int result = cqfi_next(qfi);
...
int cqfi_initial_madvise(QFi *qfi)
{
