...
#ifdef LOG_WAIT_TIME
static inline bool cqf_spin_lock(CQF *qf,
                                 volatile int *lock,
                                 uint64_t idx,
                                 uint8_t flag)
{
    ...
    if (GET_WAIT_FOR_LOCK(flag) != QF_WAIT_FOR_LOCK) {
        ...
        if (!__sync_lock_test_and_set(lock, 1)) {
            ...
            qf->runtimedata->wait_times[idx].locks_acquired_single_attempt++;
            qf->runtimedata->wait_times[idx].total_time_single +=
                BILLION * (end.tv_sec - start.tv_sec) + end.tv_nsec -
                start.tv_nsec;
        } else {
    ...
    /*clock_gettime(CLOCK_THREAD_CPUTIME_ID, &end);*/
    /*qf->runtimedata->wait_times[idx].locks_acquired_single_attempt++;*/
    /*qf->runtimedata->wait_times[idx].total_time_single += BILLION *
     * (end.tv_sec
 ...
 */
static inline bool cqf_spin_lock(volatile int *lock, uint8_t flag)
{
    if (GET_WAIT_FOR_LOCK(flag) != QF_WAIT_FOR_LOCK) {
...
static inline void cqf_spin_unlock(volatile int *lock)
{
    __sync_lock_release(lock);
    return;
}

static bool cqf_lock(CQF *qf,
    ...
    if (small) {
        ...
        if (NUM_SLOTS_TO_LOCK - hash_bucket_lock_offset <= CLUSTER_SIZE) {
            if (!cqf_spin_lock(
                ...
                cqf_spin_unlock(&qf->runtimedata->locks[hash_bucket_index /
                                                        NUM_SLOTS_TO_LOCK]);
                return false;
            }
        }
...
static void cqf_unlock(CQF *qf, uint64_t hash_bucket_index, bool small)
{
    uint64_t hash_bucket_lock_offset = hash_bucket_index % NUM_SLOTS_TO_LOCK;
    if (small) {
        if (NUM_SLOTS_TO_LOCK - hash_bucket_lock_offset <= CLUSTER_SIZE) {
...
/*static void modify_metadata(CQF *qf, uint64_t *metadata, int cnt)*/
/*{*/
/*#ifdef LOG_WAIT_TIME*/
/*cqf_spin_lock(qf, &qf->runtimedata->metadata_lock,*/
/*qf->runtimedata->num_locks, QF_WAIT_FOR_LOCK);*/
/*#else*/
/*cqf_spin_lock(&qf->runtimedata->metadata_lock, QF_WAIT_FOR_LOCK);*/
/*#endif*/
/**metadata = *metadata + cnt;*/
/*cqf_spin_unlock(&qf->runtimedata->metadata_lock);*/
/*return;*/
/*}*/
...
static inline int is_runend(const CQF *qf, uint64_t index)
{
...
static inline int is_occupied(const CQF *qf, uint64_t index)
{
...
static inline uint64_t get_slot(const CQF *qf, uint64_t index)
{
...
static inline void set_slot(const CQF *qf, uint64_t index, uint64_t value)
{
    assert(index < qf->metadata->xnslots);
    get_block(qf, index / QF_SLOTS_PER_BLOCK)
        ->slots[index % QF_SLOTS_PER_BLOCK] =
...
static inline uint64_t get_slot(const CQF *qf, uint64_t index)
{
    ...
    return (uint64_t)(
        ((*p) >> (((index % QF_SLOTS_PER_BLOCK) * QF_BITS_PER_SLOT) % 8)) &
        BITMASK(CQF_BITS_PER_SLOT));
}
...
static inline void set_slot(const CQF *qf, uint64_t index, uint64_t value)
{
    ...
    uint64_t *p =
        (uint64_t *) &get_block(qf, index / QF_SLOTS_PER_BLOCK)
            ->slots[(index % QF_SLOTS_PER_BLOCK) * QF_BITS_PER_SLOT / 8];
    uint64_t t = *p;
    uint64_t mask = BITMASK(CQF_BITS_PER_SLOT);
    uint64_t v = value;
...
static inline uint64_t get_slot(const CQF *qf, uint64_t index)
{
    ...
    return (uint64_t)(
        ((*p) >>
         (((index % QF_SLOTS_PER_BLOCK) * qf->metadata->bits_per_slot) % 8)) &
...
static inline uint64_t run_end(const CQF *qf, uint64_t hash_bucket_index);

static inline uint64_t block_offset(const CQF *qf, uint64_t blockidx)
{
...
static inline uint64_t run_end(const CQF *qf, uint64_t hash_bucket_index)
{
    ...
    if (bucket_intrablock_rank == 0) {
        if (bucket_blocks_offset <= bucket_intrablock_offset)
            ...
            return QF_SLOTS_PER_BLOCK * bucket_block_index +
                   bucket_blocks_offset - 1;
    }
...
static inline int offset_lower_bound(const CQF *qf, uint64_t slot_index)
{
    ...
    assert(CQF_SLOTS_PER_BLOCK == 64);
    if (boffset <= slot_offset) {
        const uint64_t runends =
            (b->runends[0] & BITMASK(slot_offset)) >> boffset;
        return popcnt(occupieds) - popcnt(runends);
    }
    return boffset - slot_offset + popcnt(occupieds);
}

static inline int is_empty(const CQF *qf, uint64_t slot_index)
{
    return offset_lower_bound(qf, slot_index) == 0;
}

static inline int might_be_empty(const CQF *qf, uint64_t slot_index)
{
    return !is_occupied(qf, slot_index) && !is_runend(qf, slot_index);
}

static inline int probably_is_empty(const CQF *qf, uint64_t slot_index)
{
    return get_slot(qf, slot_index) == 0 && !is_occupied(qf, slot_index) &&
           !is_runend(qf, slot_index);
}

static inline uint64_t find_first_empty_slot(CQF *qf, uint64_t from)
{
    do {
...
static inline uint64_t shift_into_b(const uint64_t a,
                                    const uint64_t b,
                                    const int bstart,
                                    const int bend,
                                    const int amount)
{
    ...
    const uint64_t b_shifted =
        ((b_shifted_mask & b) << amount) & b_shifted_mask;
...
static inline void shift_remainders(CQF *qf,
                                    ...
                                    const uint64_t empty_index)
{
    ...
    while (last_word != first_word) {
        *REMAINDER_WORD(qf, last_word) = shift_into_b(
            *REMAINDER_WORD(qf, last_word - 1), *REMAINDER_WORD(qf, last_word),
...
static inline void cqf_dump_block(const CQF *qf, uint64_t i)
{
    uint64_t j;
    ...
    for (j = 0; j < QF_SLOTS_PER_BLOCK; j++)
        printf(" %d ", (get_block(qf, i)->runends[j / 64] & (1ULL << (j % 64)))
                           ? 1
                           : 0);
    printf("\n");
...
static inline void find_next_n_empty_slots(CQF *qf,
                                           uint64_t from,
                                           uint64_t n,
                                           uint64_t *indices)
{
...
static inline void shift_slots(CQF *qf,
                               int64_t first,
                               uint64_t last,
                               uint64_t distance)
{
...
static inline void shift_runends(CQF *qf,
                                 int64_t first,
                                 uint64_t last,
                                 uint64_t distance)
{
    ...
    uint64_t noverwrites)
{
    ...
    if (ninserts > 0) {
        ...
        for (i = noverwrites; i < total_remainders - 1; i++)
            METADATA_WORD(qf, runends, overwrite_index + i) &=
                ~(1ULL << (((overwrite_index + i) % QF_SLOTS_PER_BLOCK) % 64));

        switch (operation) {
        case 0: /* insert into empty bucket */
            assert(noverwrites == 0);
            METADATA_WORD(qf, runends,
                          overwrite_index + total_remainders - 1) |=
        ...
        case 1: /* append to bucket */
            METADATA_WORD(qf, runends, overwrite_index + noverwrites - 1) &=
                ~(1ULL << (((overwrite_index + noverwrites - 1) %
                            QF_SLOTS_PER_BLOCK) %
                           64));
            METADATA_WORD(qf, runends,
                          overwrite_index + total_remainders - 1) |=
        ...
        for (i = bucket_index / QF_SLOTS_PER_BLOCK + 1;
             i <= empties[0] / QF_SLOTS_PER_BLOCK; i++) {
            while ((int64_t) npreceding_empties < ninserts &&
                   empties[ninserts - 1 - npreceding_empties] /
                           QF_SLOTS_PER_BLOCK <
                       i)
                npreceding_empties++;

            if (get_block(qf, i)->offset + ninserts - npreceding_empties <
                BITMASK(8 * sizeof(qf->blocks[0].offset)))
                get_block(qf, i)->offset += ninserts - npreceding_empties;
            else
                get_block(qf, i)->offset =
                    (uint8_t) BITMASK(8 * sizeof(qf->blocks[0].offset));
        }
    }
    ...
    uint64_t old_length)
{
    ...
    // If this is the last thing in its run, then we may need to set a new
    // runend bit
    if (is_runend(qf, overwrite_index + old_length - 1)) {
        if (total_remainders > 0) {
            // If we're not deleting this entry entirely, then it will still the
            // last entry in this run
            METADATA_WORD(qf, runends,
                          overwrite_index + total_remainders - 1) |=
                1ULL << ((overwrite_index + total_remainders - 1) % 64);
        } else if (overwrite_index > bucket_index &&
                   !is_runend(qf, overwrite_index - 1)) {
            // If we're deleting this entry entirely, but it is not the first
            // entry in this run, then set the preceding entry to be the runend
            METADATA_WORD(qf, runends, overwrite_index - 1) |=
                1ULL << ((overwrite_index - 1) % 64);
        }
    }
    ...
    while (current_distance > 0) {
        ...
        if (current_bucket <= current_slot) {
            set_slot(qf, current_slot,
                     get_slot(qf, current_slot + current_distance));
            if (is_runend(qf, current_slot) !=
                is_runend(qf, current_slot + current_distance))
                METADATA_WORD(qf, runends, current_slot) ^=
    ...
    if (old_length > total_remainders) {  // we only update offsets if we
                                          // shift/delete anything
        while (1) {
...
/*
         ...
         >3 0s:   0c...c00  for c != 0
         */
static inline uint64_t *encode_counter(CQF *qf,
                                       uint64_t remainder,
                                       uint64_t counter,
                                       uint64_t *slots)
{
...
static inline uint64_t decode_counter(const CQF *qf,
                                      uint64_t index,
                                      uint64_t *remainder,
                                      uint64_t *count)
{
 ...
 * different element
 * */
static inline uint64_t next_slot(CQF *qf, uint64_t current)
{
...
static inline int insert1(CQF *qf, __uint128_t hash, uint8_t runtime_lock)
{
    int ret_distance = 0;
    ...
    if (GET_NO_LOCK(runtime_lock) != QF_NO_LOCK) {
        if (!cqf_lock(qf, hash_bucket_index, /*small*/ true, runtime_lock))
    ...
    if (is_empty(qf, hash_bucket_index) /* might_be_empty(qf, hash_bucket_index) && runend_index == hash_bucket_index */) {
        ...
        if (is_occupied(qf, hash_bucket_index)) {
            ...
            if (current_remainder == 0) {
                ...
                if (t < runend_index && get_slot(qf, t + 1) == 0)
                    zero_terminator = t + 1; /* Three or more 0s */
                         ...
                         get_slot(qf, runstart_index + 1) == 0)
                    zero_terminator = runstart_index + 1; /* Exactly two 0s */
                /* Otherwise, exactly one 0 (i.e. zero_terminator ==
                 * runstart_index) */
            ...
            while (current_remainder < hash_remainder &&
                ...
                /* This may read past the end of the run, but the while loop
                         condition will prevent us from using the invalid result
                   in that case. */
                current_remainder = get_slot(qf, runstart_index);
            }
            ...
            if (runstart_index > runend_index) {
                ...
                /* If there's exactly one instance of this remainder. */
                       ...
                       (hash_remainder > 0 &&
                        get_slot(qf, runstart_index + 1) > hash_remainder) ||
                       (hash_remainder == 0 &&
                ...
                /* Special case for three 0s */
                       ...
                       zero_terminator == runstart_index + 2) {
                operation = 2; /* Insert */
                insert_index = runstart_index + 1;
                new_value = 1;
                ...
                do {
                    ...
                    // Convert a leading 0 (which is special) to a normal
                    // encoded digit
                    if (digit == 0) {
                        digit++;
                    ...
                    if (digit == current_remainder)
                        digit =
                            (digit + 1) & BITMASK(qf->metadata->bits_per_slot);
                    if (digit == 0) {
                        digit++;
                        carry = 1;
                    }
                ...
                if (insert_index == runstart_index &&
                    (carry > 0 ||
                     (current_remainder != 0 && digit >= current_remainder))) {
                    operation = 2; /* insert */
                    insert_index = runstart_index + 1;
                    if (!carry) /* To prepend a 0 before the counter if the MSD
                                   is greater than the rem */
                        new_value = 0;
                    else if (carry) { /* Increment the new value because we
                                         don't use 0 to encode counters */
                        new_value = 2;
                        /* If the rem is greater than or equal to the new_value
                         * then fail*/
        ...
        if (operation >= 0) {
            uint64_t empty_slot_index =
                find_first_empty_slot(qf, runend_index + 1);
    ...
    if (GET_NO_LOCK(runtime_lock) != QF_NO_LOCK) {
        cqf_unlock(qf, hash_bucket_index, /*small*/ true);
    }

    return ret_distance;
}

static inline int insert(CQF *qf,
                         __uint128_t hash,
                         uint64_t count,
                         uint8_t runtime_lock)
{
    ...
    uint64_t hash_bucket_index = hash >> qf->metadata->bits_per_slot;
    uint64_t hash_bucket_block_offset = hash_bucket_index % QF_SLOTS_PER_BLOCK;
    /*uint64_t hash_bucket_lock_offset  = hash_bucket_index %
     * NUM_SLOTS_TO_LOCK;*/

    if (GET_NO_LOCK(runtime_lock) != QF_NO_LOCK) {
        if (!cqf_lock(qf, hash_bucket_index, /*small*/ false, runtime_lock))
            return QF_COULDNT_LOCK;
    }
    ...
    /* Empty slot */
    if (might_be_empty(qf, hash_bucket_index) &&
        ...
        bool ret;
        if (!is_occupied(qf, hash_bucket_index)) { /* Empty bucket, but its slot
                                                      is occupied. */
            uint64_t *p =
                encode_counter(qf, hash_remainder, count, &new_values[67]);
            ret =
                insert_replace_slots_and_shift_remainders_and_runends_and_offsets(
                    qf, 0, hash_bucket_index, runstart_index, p,
                    &new_values[67] - p, 0);
            if (!ret)
                return QF_NO_SPACE;
            modify_metadata(&qf->runtimedata->pc_ndistinct_elts, 1);
            ...
            current_end = decode_counter(qf, runstart_index, &current_remainder,
                                         &current_count);
            while (current_remainder < hash_remainder &&
                   !is_runend(qf, current_end)) {
                runstart_index = current_end + 1;
                current_end = decode_counter(
                    qf, runstart_index, &current_remainder, &current_count);
            }

            /* If we reached the end of the run w/o finding a counter for this
               remainder, then append a counter for this remainder to the run.
             */
            if (current_remainder < hash_remainder) {
                ...
                ret =
                    insert_replace_slots_and_shift_remainders_and_runends_and_offsets(
                        qf, 1, /* Append to bucket */
                        hash_bucket_index, current_end + 1, p,
                        &new_values[67] - p, 0);
                if (!ret)
                    return QF_NO_SPACE;
                modify_metadata(&qf->runtimedata->pc_ndistinct_elts, 1);
                ...
                uint64_t *p = encode_counter(
                    qf, hash_remainder, current_count + count, &new_values[67]);
                ret =
                    insert_replace_slots_and_shift_remainders_and_runends_and_offsets(
                        qf, is_runend(qf, current_end) ? 1 : 2,
                        hash_bucket_index, runstart_index, p,
                        &new_values[67] - p, current_end - runstart_index + 1);
                if (!ret)
                    return QF_NO_SPACE;
                ret_distance = runstart_index - hash_bucket_index;
    ...
    if (GET_NO_LOCK(runtime_lock) != QF_NO_LOCK) {
        cqf_unlock(qf, hash_bucket_index, /*small*/ false);
    }

    return ret_distance;
}

inline static int _remove(CQF *qf,
                          __uint128_t hash,
                          uint64_t count,
                          uint8_t runtime_lock)
{
    ...
    uint64_t *p = encode_counter(
        qf, hash_remainder, count > current_count ? 0 : current_count - count,
        &new_values[67]);
    ret_numfreedslots =
        remove_replace_slots_and_shift_remainders_and_runends_and_offsets(
            qf, only_item_in_the_run, hash_bucket_index, runstart_index, p,
            &new_values[67] - p, current_end - runstart_index + 1);
                  ...
                  uint64_t buffer_len)
{
    ...
    pc_init(&qf->runtimedata->pc_nelts, (int64_t *) &qf->metadata->nelts, 8,
            100);
    pc_init(&qf->runtimedata->pc_ndistinct_elts,
            (int64_t *) &qf->metadata->ndistinct_elts, 8, 100);
    pc_init(&qf->runtimedata->pc_noccupied_slots,
            (int64_t *) &qf->metadata->noccupied_slots, 8, 100);
    /* initialize container resize */
    qf->runtimedata->auto_resize = 0;
    qf->runtimedata->container_resize = cqf_resize_malloc;
...
{
    uint64_t total_num_bytes =
        cqf_init(qf, nslots, key_bits, value_bits, hash, seed, NULL, 0);
    ...
    uint64_t init_size = cqf_init(qf, nslots, key_bits, value_bits, hash, seed,
...
{
    CQF new_qf;
    if (!cqf_malloc(&new_qf, nslots, qf->metadata->key_bits,
                    qf->metadata->value_bits, qf->metadata->hash_mode,
                    qf->metadata->seed))
        return -1;
    if (qf->runtimedata->auto_resize)
        cqf_set_auto_resize(&new_qf, true);

    // copy keys from qf into new_qf
    QFi cqfi;
    cqf_iterator_from_position(qf, &cqfi, 0);
    int64_t ret_numkeys = 0;
    do {
        uint64_t key, value, count;
        cqfi_get_hash(&cqfi, &key, &value, &count);
        cqfi_next(&cqfi);
        int ret =
            cqf_insert(&new_qf, key, value, count, QF_NO_LOCK | QF_KEY_IS_HASH);
        if (ret < 0) {
    ...
    cqf_free(qf);
    memcpy(qf, &new_qf, sizeof(CQF));

    return ret_numkeys;
}

uint64_t cqf_resize(CQF *qf, uint64_t nslots, void *buffer, uint64_t buffer_len)
{
    CQF new_qf;
    ...
    uint64_t init_size = cqf_init(
        &new_qf, nslots, qf->metadata->key_bits, qf->metadata->value_bits,
        qf->metadata->hash_mode, qf->metadata->seed, buffer, buffer_len);
               ...
               uint8_t flags)
{
    ...
    if (cqf_get_num_occupied_slots(qf) >= qf->metadata->nslots * 0.95) {
        if (qf->runtimedata->auto_resize) {
            fprintf(stdout, "Resizing the CQF.\n");
            if (qf->runtimedata->container_resize(
                    qf, qf->metadata->nslots * 2) < 0) {
    ...
    if (ret == QF_NO_SPACE || ret > DISTANCE_FROM_HOME_SLOT_CUTOFF) {
        float load_factor =
            cqf_get_num_occupied_slots(qf) / (float) qf->metadata->nslots;
                  ...
                  uint8_t flags)
{
    ...
    uint64_t cur_count = cqf_count_key_value(qf, key, value, flags);
    ...
    if (delta == 0)
        ...
        ret = cqf_insert(qf, key, value, delta, flags);
    else
        ret = cqf_remove(qf, key, value, labs(delta), flags);
...
{
    uint64_t count = cqf_count_key_value(qf, key, value, flags);
    if (count == 0)
        return true;
...
uint64_t cqf_query(const CQF *qf, uint64_t key, uint64_t *value, uint8_t flags)
{
    ...
    // Get the remainder / quotient part of hashed bits
    ...
    // Find start index of target run
    int64_t runstart_index =
        hash_bucket_index == 0 ? 0 : run_end(qf, hash_bucket_index - 1) + 1;
    if (runstart_index < hash_bucket_index)
        runstart_index = hash_bucket_index;
    ...
    uint64_t current_remainder, current_count, current_end;
    do {
        // Parsing slots to get one element (remainder & count pair)
        current_end = decode_counter(qf, runstart_index, &current_remainder,
                                     &current_count);
...
int64_t cqf_iterator_from_position(const CQF *qf, QFi *cqfi, uint64_t position)
{
    if (position == 0xffffffffffffffff) {
        cqfi->current = 0xffffffffffffffff;
        cqfi->qf = qf;
        return QFI_INVALID;
    }
    ...
    cqfi->num_clusters = 0;
    cqfi->run = position;
    cqfi->current = position == 0 ? 0 : run_end(cqfi->qf, position - 1) + 1;
    if (cqfi->current < position)
        cqfi->current = position;

#ifdef LOG_CLUSTER_LENGTH
    cqfi->c_info = (cluster_data *) calloc(qf->metadata->nslots / 32,
                                           sizeof(cluster_data));
    if (cqfi->c_info == NULL) {
        perror("Couldn't allocate memory for c_info.");
        exit(EXIT_FAILURE);
    }
    cqfi->cur_start_index = position;
    cqfi->cur_length = 1;
#endif

    if (cqfi->current >= qf->metadata->nslots)
        return QFI_INVALID;
    return cqfi->current;
}

int64_t cqf_iterator_key_value(const CQF *qf,
                               QFi *cqfi,
                               uint64_t key,
                               uint64_t value,
                               uint8_t flags)
{
    ...
    if (is_occupied(qf, hash_bucket_index)) {
        ...
        if (flag) {
            cqfi->run = hash_bucket_index;
            cqfi->current = runstart_index;
        }
    }
...
static int cqfi_get(const QFi *cqfi,
                    uint64_t *key,
                    uint64_t *value,
                    uint64_t *count)
{
    ...
    decode_counter(cqfi->qf, cqfi->current, &current_remainder, &current_count);

    *value = current_remainder & BITMASK(cqfi->qf->metadata->value_bits);
    current_remainder = current_remainder >> cqfi->qf->metadata->value_bits;
    *key = (cqfi->run << cqfi->qf->metadata->key_remainder_bits) |
           current_remainder;
    *count = current_count;
...
{
    *key = *value = *count = 0;
    int ret = cqfi_get(cqfi, key, value, count);
    if (ret == 0) {
        if (cqfi->qf->metadata->hash_mode == QF_HASH_DEFAULT) {
            *key = 0;
            *value = 0;
            *count = 0;
            return QF_INVALID;
        } else if (cqfi->qf->metadata->hash_mode == QF_HASH_INVERTIBLE)
            *key = hash_64i(*key, BITMASK(cqfi->qf->metadata->key_bits));
    }
...
{
    *key = *value = *count = 0;
    return cqfi_get(cqfi, key, value, count);
}
...
int cqfi_next(QFi *cqfi)
{
    if (cqfi_end(cqfi))
        ...
        uint64_t current_remainder, current_count;
        cqfi->current = decode_counter(cqfi->qf, cqfi->current,
                                       &current_remainder, &current_count);

        if (!is_runend(cqfi->qf, cqfi->current)) {
            cqfi->current++;
#ifdef LOG_CLUSTER_LENGTH
            cqfi->cur_length++;
#endif
            ...
            uint64_t old_current = cqfi->current;
#endif
            uint64_t block_index = cqfi->run / QF_SLOTS_PER_BLOCK;
            uint64_t rank =
                bitrank(get_block(cqfi->qf, block_index)->occupieds[0],
                        cqfi->run % QF_SLOTS_PER_BLOCK);
            uint64_t next_run =
                bitselect(get_block(cqfi->qf, block_index)->occupieds[0], rank);
            if (next_run == 64) {
                rank = 0;
                while (next_run == 64 &&
                       block_index < cqfi->qf->metadata->nblocks) {
                    block_index++;
                    next_run = bitselect(
                        get_block(cqfi->qf, block_index)->occupieds[0], rank);
                }
            }
            if (block_index == cqfi->qf->metadata->nblocks) {
                /* set the index values to max. */
                cqfi->run = cqfi->current = cqfi->qf->metadata->xnslots;
                return QFI_INVALID;
            }
            cqfi->run = block_index * QF_SLOTS_PER_BLOCK + next_run;
            cqfi->current++;
            if (cqfi->current < cqfi->run)
                cqfi->current = cqfi->run;
#ifdef LOG_CLUSTER_LENGTH
            if (cqfi->current > old_current + 1) { /* new cluster. */
                if (cqfi->cur_length > 10) {
                    cqfi->c_info[cqfi->num_clusters].start_index =
                        cqfi->cur_start_index;
                    cqfi->c_info[cqfi->num_clusters].length = cqfi->cur_length;
                    cqfi->num_clusters++;
                }
                cqfi->cur_start_index = cqfi->run;
                cqfi->cur_length = 1;
            } else {
                cqfi->cur_length++;
            }
...
{
    if (cqfi->current >=
        cqfi->qf->metadata->xnslots /*&& is_runend(cqfi->qf, cqfi->current)*/)
        return true;
    return false;
}
...
{
    QFi cqfia, cqfib;
    cqf_iterator_from_position(qfa, &cqfia, 0);
    cqf_iterator_from_position(qfb, &cqfib, 0);
    ...
    if (qfa->metadata->hash_mode != qfc->metadata->hash_mode &&
        ...
        fprintf(stderr,
                "Output QF and input QFs do not have the same hash mode or "
                "seed.\n");
        exit(1);
    }

    uint64_t keya, valuea, counta, keyb, valueb, countb;
    cqfi_get_hash(&cqfia, &keya, &valuea, &counta);
    cqfi_get_hash(&cqfib, &keyb, &valueb, &countb);
    do {
        if (keya < keyb) {
            cqf_insert(qfc, keya, valuea, counta, QF_NO_LOCK | QF_KEY_IS_HASH);
            cqfi_next(&cqfia);
            cqfi_get_hash(&cqfia, &keya, &valuea, &counta);
        } else {
            cqf_insert(qfc, keyb, valueb, countb, QF_NO_LOCK | QF_KEY_IS_HASH);
            cqfi_next(&cqfib);
            cqfi_get_hash(&cqfib, &keyb, &valueb, &countb);
        }
    } while (!cqfi_end(&cqfia) && !cqfi_end(&cqfib));

    if (!cqfi_end(&cqfia)) {
        do {
            cqfi_get_hash(&cqfia, &keya, &valuea, &counta);
            cqf_insert(qfc, keya, valuea, counta, QF_NO_LOCK | QF_KEY_IS_HASH);
        } while (!cqfi_next(&cqfia));
    }
    if (!cqfi_end(&cqfib)) {
        do {
            cqfi_get_hash(&cqfib, &keyb, &valueb, &countb);
            cqf_insert(qfc, keyb, valueb, countb, QF_NO_LOCK | QF_KEY_IS_HASH);
        } while (!cqfi_next(&cqfib));
    }
}
...
void cqf_multi_merge(const CQF *cqf_arr[], int nqf, CQF *qfr)
{
    ...
    for (i = 0; i < nqf; i++) {
        if (cqf_arr[i]->metadata->hash_mode != qfr->metadata->hash_mode &&
            cqf_arr[i]->metadata->seed != qfr->metadata->seed) {
            fprintf(stderr,
                    "Output QF and input QFs do not have the same hash mode or "
                    "seed.\n");
            exit(1);
        }
        cqf_iterator_from_position(cqf_arr[i], &cqfi_arr[i], 0);
    }

    DEBUG_CQF("Merging %d CQFs\n", nqf);
    for (i = 0; i < nqf; i++) {
        DEBUG_CQF("CQF %d\n", i);
        DEBUG_DUMP(cqf_arr[i]);
    }
    ...
    while (nqf > 1) {
        ...
        uint64_t counts[nqf];
        for (i = 0; i < nqf; i++)
            cqfi_get_hash(&cqfi_arr[i], &keys[i], &values[i], &counts[i]);
        ...
        do {
            ...
            cqf_insert(qfr, keys[smallest_idx], values[smallest_idx],
                       counts[smallest_idx], QF_NO_LOCK | QF_KEY_IS_HASH);
            cqfi_next(&cqfi_arr[smallest_idx]);
            cqfi_get_hash(&cqfi_arr[smallest_idx], &keys[smallest_idx],
                          &values[smallest_idx], &counts[smallest_idx]);
        } while (!cqfi_end(&cqfi_arr[smallest_idx]));

        /* remove the qf that is exhausted from the array */
        if (smallest_idx < nqf - 1)
            memmove(&cqfi_arr[smallest_idx], &cqfi_arr[smallest_idx + 1],
                    (nqf - smallest_idx - 1) * sizeof(cqfi_arr[0]));
        nqf--;
    }
    if (!cqfi_end(&cqfi_arr[0])) {
        uint64_t iters = 0;
        do {
            uint64_t key, value, count;
            cqfi_get_hash(&cqfi_arr[0], &key, &value, &count);
            cqf_insert(qfr, key, value, count, QF_NO_LOCK | QF_KEY_IS_HASH);
            cqfi_next(&cqfi_arr[0]);
            iters++;
        } while (!cqfi_end(&cqfi_arr[0]));
...
uint64_t cqf_inner_product(const CQF *qfa, const CQF *qfb)
{
    ...
    if (qfa->metadata->total_size_in_bytes >
    ...
    cqf_iterator_from_position(cqf_disk, &cqfi, 0);
    do {
        uint64_t key = 0, value = 0, count = 0;
        uint64_t count_mem;
        cqfi_get_hash(&cqfi, &key, &value, &count);
        if ((count_mem = cqf_count_key_value(cqf_mem, key, 0, QF_KEY_IS_HASH)) >
            0) {
            acc += count * count_mem;
        }
    } while (!cqfi_next(&cqfi));

    return acc;
}

/* find cosine similarity between two QFs. */
void cqf_intersect(const CQF *qfa, const CQF *qfb, CQF *qfr)
{
    QFi cqfi;
    const CQF *cqf_mem, *cqf_disk;
    ...
    do {
        uint64_t key = 0, value = 0, count = 0;
        cqfi_get_hash(&cqfi, &key, &value, &count);
        if (cqf_count_key_value(cqf_mem, key, 0, QF_KEY_IS_HASH) > 0)
            cqf_insert(qfr, key, value, count, QF_NO_LOCK | QF_KEY_IS_HASH);
    } while (!cqfi_next(&cqfi));
}
...
{
    return sqrt(cqf_inner_product(qf, qf));
}
